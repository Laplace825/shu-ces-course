...
void Polynomial::sort(
    const std::function<bool(const Term &, const Term &)> &cmp) noexcept
{
    std::sort(poly.begin(), poly.end(), cmp);
}

Polynomial::Polynomial() = default;
Polynomial::Polynomial(const std::initializer_list<Term> &list) : poly(list)
{
    // 按指数递增排序
...
{
    for (auto &t : other.poly) t.setCoef(-t.getCoef());
    return *this + other;
}
                       ...
                       const std::size_t &width) const
{
    ...
    if ((t + 1) == poly.end()) std::cout << '\n';
}

inline bool Polynomial::empty() const noexcept { return poly.empty(); }

inline size_t Polynomial::size() const noexcept { return poly.size(); }

double Polynomial::cal(double x) const noexcept
{
    // 递归计算多项式的值
    double result = 0;
    for (auto &t : poly) result += t.cal(x);
    return result;
}

auto Polynomial::begin() noexcept { return poly.begin(); }

auto Polynomial::end() noexcept { return poly.end(); }

auto Polynomial::cbegin() const noexcept { return poly.cbegin(); }

auto Polynomial::cend() const noexcept { return poly.cend(); }
